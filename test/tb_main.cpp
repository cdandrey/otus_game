...
const std::vector<otg::PropertyKey> listPropertys {otg::PositionProperty::key,
...
TEST(tb_main, move)
{
	...
	const auto actualPosition = PositionProperty::cast(tank->getProperty(PositionProperty::key).value_or(PositionProperty::type {})).value();
...
TEST(tb_main, rotate)
{
	...
	const auto actualDirection = DirectionProperty::cast(tank->getProperty(DirectionProperty::key).value_or(DirectionProperty::type {})).value();
